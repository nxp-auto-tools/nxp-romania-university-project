...
bool b_calculateIntValue(const pdb_timer_config_t *pdbConfig, uint32_t uSec, uint16_t *intVal) {
    ...
    CLOCK_SYS_GetFreq(CORE_CLOCK, &ui8PdbFrequency);
    ui8PdbFrequency /= 1000000U;

    /* calculate the interrupt value based on time and clock settings */
    ui16Temp = (ui8PdbFrequency * uSec) / (ui8PdbPrescaler * ui8PdbPrescalerMult);

    /* check if the result fits in a 16-bit value */
    if ((ui16Temp == 0U) || (ui16Temp >= (1U << 16))) {
        bResultValid = false;
        (*intVal) = 0U;
    } else {
        bResultValid = true;
        (*intVal) = (uint16_t) ui16Temp;
    }
