...
void v_updateSpeed(uint8_t ui8Acceleration) {
    ...
    /* TODO: Calculate gear factor to simulate how lower gears accelerate faster
     * This creates values like: 1st gear = 1.0, 2nd = 0.9, 3rd = 0.8, etc.
     * Lower gears (1st, 2nd) will have higher factors = faster acceleration
     * Higher gears (5th, 6th) will have lower factors = slower acceleration
...
void v_calculateRPM(uint8_t ui8Acceleration){
    ...
    /* TODO: Calculate base RPM based on acceleration using exponential curve
     * This creates a realistic RPM curve where RPM increases quickly at first, then levels off
     * - At 0% acceleration: RPM stays at IDLE_RPM (1900)
     * - At 50% acceleration: RPM is around 4500
     * - At 100% acceleration: RPM approaches MAX_RPM (7000)
     * Hint: Use the exp() function from math.h (already included via motor.h) */
    rpmBase = 0.0F;
...
void v_displayRPM(ftm_instanceConfig_t* ftmMotorInstance){
    /* TODO: Convert RPM to PWM duty cycle (0 to 100%)
     * This function scales RPM (0 to 7000) to duty cycle (0 to 0x8000)
     * Example: 3500 RPM → 0x4000 duty cycle (50% power to motor) */
    ui16MotorDutyCycle = 0U;
